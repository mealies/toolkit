...
var slugTests = []struct {
	...
	{name: "japanese string and roman character", s: "hello world ハローワールド", expected: "hello-world", errorExpected: false},
}
...
var jsonTests = []struct {
	...
	{name: "file too large", json: `{"foo": "1"}`, errorExpected: true, maxSize: 5, allowUnknown: true},
...
func TestTools_WriteJSON(t *testing.T) {
	...
	if err != nil {
		t.Errorf("failed ot write JSON: %v", err)
	}
}
...
func TestTools_ErrorJSON(t *testing.T) {
	...
	if err != nil {
		t.Error("reeived error when decoding JSON", err)
	}
	...
	if rr.Code != http.StatusServiceUnavailable {
		t.Errorf("wrong status code reutrned; expected 503, but got %d", rr.Code)
	}
}
