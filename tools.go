...
// UploadedFile is a convenience method that calls UploadFiles, but expects only one file to be in the upload
...
// UploadFiles uploads one of more files to a specified directory, and gives the files a random name.
// It returns a slice containing the newly named files, the original file names, the size of the file
// and potentially an error.  If the optional last param is set to true, then we will not rename the files, but
...
// Slugify is a very simple way to create a slug is from a string
...
// PushJSONToRemote posts arbitrary data to some URI as JSON, and returns response, status code and error, if any.
// the paramater, client, is optional.  If none is specified, we use the standard http.Client
